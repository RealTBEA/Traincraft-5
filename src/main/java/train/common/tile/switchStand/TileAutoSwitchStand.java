        ...
        if (!world.isRemote) {
            if (updateTicks % 20 == 0) {
                ...
                if (!this.world.isAirBlock(tempBlockPos))
                {
                    ...
                    if (block != null) {
                        EntityItem entityitem = new EntityItem(world, this.getPos().getX(), this.getPos().getY() + 1,  this.getPos().getZ(), new ItemStack(Item.getItemFromBlock(TCBlocks.autoSwtichStand), 1));
                        float f3 = 0.05F;
